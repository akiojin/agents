...
> {
  ...
  constructor(private config: Config) {
    ...
    super(
      ...
      'ReadManyFiles',
      `Reads content from multiple files specified by paths or glob patterns within a configured target directory. For text files, it concatenates their content into a single string. It is primarily designed for text-based files. However, it can also process image (e.g., .png, .jpg), PDF (.pdf), and Office files (Word, Excel, PowerPoint) if their file names or extensions are explicitly included in the 'paths' argument. Office files are parsed into readable text format, while other non-text files are included in a format suitable for model consumption (e.g., base64 encoded).
