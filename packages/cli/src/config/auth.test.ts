...
describe('validateAuthMethod', () => {
  ...
  describe('USE_GEMINI', () => {
    it('should return null if AGENTS_API_KEY is set', () => {
      process.env.AGENTS_API_KEY = 'test-key';
      expect(validateAuthMethod(AuthType.USE_GEMINI)).toBeNull();
    });

    it('should return an error message if AGENTS_API_KEY is not set', () => {
      expect(validateAuthMethod(AuthType.USE_GEMINI)).toBe(
        'AGENTS_API_KEY environment variable not found. Add that to your environment and try again (no reload needed if using .env)!',
      );
    });
  });
