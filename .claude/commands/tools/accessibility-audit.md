...
class ColorContrastAnalyzer {
  ...
  async analyzePageContrast(page) {
    ...
    for (const element of elements) {
      ...
      const requiredContrast = isLargeText ? this.wcagLevels.AA.large : this.wcagLevels.AA.normal;
  ...
  generateColorRecommendation(foreground, background, targetRatio) {
    ...
    const darkerFg = this.adjustColorForContrast(foreground, background, targetRatio, 'darken');
    if (darkerFg) {
      suggestions.push({
    ...
    const lighterBg = this.adjustColorForContrast(background, foreground, targetRatio, 'lighten');
    if (lighterBg) {
      suggestions.push({
...
class KeyboardNavigationTester {
  ...
  async detectKeyboardTraps(page, results) {
    ...
    const trapSelectors = ['div[role="dialog"]', '.modal', '.dropdown-menu', '[role="menu"]'];
  ...
  async testInteractiveElements(page, results) {
    ...
    const clickableElements = await page.evaluate(() => {
      ...
      elements.forEach((el) => {
        ...
        const isNotNativelyClickable = !['a', 'button', 'input', 'select', 'textarea'].includes(
          el.tagName.toLowerCase(),
        );
...
const AccessibleButton = ({ children, onClick, ariaLabel, ariaPressed, disabled, ...props }) => {
  return (
    <button
      onClick={onClick}
