...
export class MCPManager extends EventEmitter {
    ...
    getServerForTool(toolName) {
        ...
        if (toolName.startsWith('serena_')) {
            const serenaServer = Array.from(this.servers.keys()).find(name => name.includes('serena') || name === 'serena');
            if (serenaServer) {
        ...
        if (toolName.startsWith('mcp__')) {
            ...
            const matchingServer = Array.from(this.servers.keys()).find(name => name.includes(serverHint));
            if (matchingServer) {
     ...
     * サーバーの初期化状態を更新
     */
    updateServerStatus(serverName, status, error, toolCount) {
