...
export class InternalBash {
  ...
  private validateCommand(command: string): { allowed: boolean; reason?: string } {
    ...
    if (this.config.allowedCommands.length > 0) {
      const isAllowed = this.config.allowedCommands.some(allowed => 
        baseCommand.startsWith(allowed) || command.startsWith(allowed)
      );
   ...
   * 作業ディレクトリを検証
   */
  private validateWorkingDirectory(cwd?: string): { allowed: boolean; validatedPath?: string; reason?: string } {
    if (!cwd) {
      return { 
        allowed: true, 
  ...
  private filterEnvironmentVariables(env?: Record<string, string>): Record<string, string> {
    ...
    const filteredEnv: Record<string, string> = { ...process.env };
   ...
   * コマンドを実行（同期的、対話式）
   */
  async executeCommandInteractive(
    command: string,
    options?: {
      cwd?: string;
      env?: Record<string, string>;
