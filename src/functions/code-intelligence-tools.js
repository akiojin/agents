...
import { createSymbolIndex } from '../../packages/core/src/code-intelligence/symbol-index.js';
import { createDefaultLSPClient } from '../../packages/core/src/code-intelligence/lsp-client.js';
/**
 * シンボルインデックスの管理
 */
class CodeIntelligenceService {
    ...
    lspClient;
    currentProjectPath;
    async getSymbolIndex(projectPath) {
 ...
 */
export function createCodeIntelligenceFunctions() {
    const functions = [];
            ...
            try {
                ...
                for (const symbol of symbols) {
                    ...
                    if (params.include_references) {
                        ...
                        symbolInfo.references = references.map(ref => ({
    ...
    functions.push({
        ...
        handler: async (params) => {
            try {
                const symbolIndex = await codeIntelligenceService.getSymbolIndex();
                const references = await symbolIndex.findReferences(params.symbol_name, params.file_path);
